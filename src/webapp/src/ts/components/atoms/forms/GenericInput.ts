...
export abstract class GenericInput<VT, T extends GenericInputAttrs<VT>> implements m.ClassComponent<T> {
  ...
  protected setValue(value:string) {
    this.refHolder[this.refProperty] = this.convertValueFromString?this.convertValueFromString(value):value;
  }
  protected getValueAsString():string {
    let v = this.refHolder[this.refProperty]
    return this.convertValueToString?this.convertValueToString(v):(""+v);
  }
};
