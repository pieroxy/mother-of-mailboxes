 ...
 * mom-rules/&lt;MATCHER_TYPE&gt;/&lt;ACTION_TYPE&gt;/&lt;key&gt; (e.g. mom-rules/FROM_EQUALS/MOVE_TO/Spam)
 ...
 * entry point onto the same mechanism: a single {@code mom-rules/<name>} folder bound to one
...
public class RuleLearner {
   ...
   *                              — only {@code mom-rules/Done} and any configured shortcut
    ...
    for (LearningShortcutConfiguration shortcut : shortcuts) {
      ...
      if (isReservedName(name)) {
        throw new IllegalArgumentException("learningShortcuts name \"" + name
                + "\" collides with a reserved mom-rules/ folder name");
      }
      ...
      if (matcher.getKey() != null || matcher.getKeys() != null || matcher.getChildren() != null) {
        throw new IllegalArgumentException("learningShortcuts \"" + name
                + "\": matcher must only set type — its key is extracted from each example, same as in mom-rules/"
                + matcher.getType() + "/...");
      }
