...
/**
 * Replays the rule catalog against messages dropped by hand into mom-rules/ToProcess (e.g. a
 ...
 * it — it's filed into mom-rules/Done (the same folder used by
 * {@link net.pieroxy.mom.learning.RuleLearner}), for the user to take a look at, since they're
...
public class ManualReprocessor {
  ...
  private final static String ROOT_FOLDER = "mom-rules";
  ...
  /** Creates mom-rules/ToProcess if needed, ready to receive messages dropped by hand. */
  ...
  private void reprocess(Message message) {
    try {
      ...
      boolean matched = RuleHelper.processRules(ruleCatalog.get(), ruleCatalog.getLearnedRulesFallback(), message, LOGGER,
          ROOT_FOLDER + "/" + TO_PROCESS_FOLDER, ruleCatalog.getContext());
